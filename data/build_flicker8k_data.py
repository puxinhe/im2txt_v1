...
# 图片文件路径
tf.flags.DEFINE_string("train_image_dir", "./flicker8k/images/",
                       "Training image directory.")
tf.flags.DEFINE_string("val_image_dir", "./flicker8k/images/",
                       "Validation image directory.")
tf.flags.DEFINE_string("test_image_dir", "./flicker8k/images/",
                       "Test image directory.")

# 图片文件名信息文件路径
tf.flags.DEFINE_string("train_text_file", "./flicker8k/Flickr_8k.trainImages.txt",
                       "Training text file.")
tf.flags.DEFINE_string("val_text_file", "./flicker8k/Flickr_8k.devImages.txt",
                       "Validation text file.")
tf.flags.DEFINE_string("test_text_file", "./flicker8k/Flickr_8k.testImages.txt",
                       "Test text file.")

#标注信息文件路径
tf.flags.DEFINE_string("captions_file", "./flicker8k/Flickr8k.token.txt",
                       "Captions text file.")

# 输出路径
tf.flags.DEFINE_string("output_dir", "./flicker8k/tfrecord-data/", "Output data directory.")

# TFRecord文件中的shards数量
tf.flags.DEFINE_integer("train_shards", 12,
...
tf.flags.DEFINE_string("word_counts_output_file", "./flicker8k/word_counts.txt",
...
def _load_and_process_metadata(text_file, image_dir):
  ...
  with tf.gfile.FastGFile(FLAGS.captions_file, "r") as f:
    doc_token = f.read().split('\n')

  with tf.gfile.FastGFile(text_file, "r") as f:
    doc_split = f.read().split('\n')

  id_to_filename = [(int(x.split('_')[0]), x) for x in doc_split]
  id_to_captions = {}
  for line in doc_token:
    token = line.split('#')
    base_filename = token[0]
    if base_filename in doc_split:
        image_id = int(base_filename.split('_')[0])
        caption = ' '.join((token[1].split())[1:])
  ...
  print("Finished processing %d captions for %d images in %s" %
        (num_captions, len(id_to_filename), text_file))
...
def main(unused_argv):
  ...
  train_dataset = _load_and_process_metadata(FLAGS.train_text_file,
                                                    FLAGS.train_image_dir)
  val_dataset = _load_and_process_metadata(FLAGS.val_text_file,
                                                  FLAGS.val_image_dir)
  test_dataset = _load_and_process_metadata(FLAGS.test_text_file,
