...
#mpl.use('Agg')
#mpl.get_backend() #可以看到是Agg, 这时plt.imshow无法显示图片，必须Qt5Agg才行
...
class GraphLoader(object):
  ...
  def open_session(self, graph):
    ...
    return tf.Session(graph=graph,config=sess_config)
...
class LSTMDecoder(GraphLoader):
  ...
  def show_caption(self, caption, img):
    plt.figure(figsize=(20,15))
    plt.imshow(img)
    plt.axis('off')
    plt.title(caption['caption'],fontsize='xx-large')
    plt.show()
  ...
  def show_attention(self, caption,alphas, image_np, save_path):
    ...
    plt.imshow(image_np)
    plt.axis('off')
    #plt.title(caption['caption'],fontsize='small')
    ...
    for idx in range(len(cap)):
      ...
      lab = cap[idx]
      plt.text(0, 1, lab, backgroundcolor='white', color='black', fontsize=18)
      plt.imshow(image_np)
      plt.imshow(alpha_image, alpha=0.8)
      plt.set_cmap(cm.Greys_r)
      plt.axis('off')
      plt.subplots_adjust(left=0.02, bottom=0.02, right=0.98, top=0.88, hspace=0.05, wspace=0.05)
